...
describe("Components / Floating Label", () => {
  ...
  it("should update the input value correctly", () => {
    const input: HTMLInputElement | null = render(<FloatingLabel buttonStyle={"filled"} label={"Label"}
                                  defaultValue={"my name is"} />).getByTestId("floating-label") as HTMLInputElement;
    expect(input.value).toBe("my name is");
  })

  it("should be disabled", () => {
    const input = render(<FloatingLabel buttonStyle={"filled"} label={"Label"} disabled={true}/>).getByTestId("floating-label");
